...
#'   \item{**team_abbr**}{ - The team abbreviation. Should use ESPN's abbreviation labels.}
#'   \item{`alpha = NULL`}{ - The alpha channel, i.e. transparency level, as a numerical value between 0 and 1.}
#'   \item{`color = NULL`}{ - The image will be colorized with this color. Use the special character `"b/w"` to set it to black and white. For more information on valid color names in ggplot2 see <https://ggplot2.tidyverse.org/articles/ggplot2-specs.html?q=colour#colour-and-fill>}
...
#' # also set a custom fill color for the non "A" teams
#' df$color <- ifelse(matches, NA, "gray")
#'
...
#'   theme_void()
#'
#' # apply alpha and color via an aesthetic from inside the dataset `df`
#' # please note that you have to add scale_alpha_identity() as well as
#' # scale_color_identity() to use the alpha and color values in your dataset!
#' ggplot(df, aes(x = a, y = b)) +
#'   geom_mlb_logos(aes(team_abbr = teams, alpha = alpha, color = color), width = 0.075) +
#'   geom_label(aes(label = teams), nudge_y = -0.35, alpha = 0.5) +
#'   scale_alpha_identity() +
...
GeomMLBlogo <- ggplot2::ggproto(
  ...
  default_aes = ggplot2::aes(
    alpha = NULL, color = NULL, angle = 0, hjust = 0.5,
    vjust = 0.5, width = 1.0, height = 1.0
  ),
  draw_panel = function(data, panel_params, coord, na.rm = FALSE) {
    data <- coord$transform(data, panel_params)

    grobs <- lapply(seq_along(data$team_abbr), build_grobs, alpha = data$alpha, color = data$color, data = data, type = "teams")
